...
class DateSerializerField(serializers.DateField):
    def to_internal_value(self, value):
        value = value.strip()
        try:
            value = timezone.datetime.strptime(value, '%b %d %Y').strftime('%Y-%m-%d')
...
class ChoicesField(serializers.Field):
    def __init__(self, choices, allow_custom = False, **kwargs):
        ...
        for choice in choices:
            if len(choice) != 2:
                raise TypeError("choices must of type list or tuple with 2 columns")
    ...
    def to_internal_value(self, value):
        try:
            ...
            pass
        raise serializers.ValidationError("Invalid field value {}".format(value))
    ...
    def __init__(self, choices, allow_custom = False, flat = False, delimeter = ",", **kwargs):
        ...
        for choice in choices:
            ...
            if delimeter in choice[1]:
                raise ValueError("Invalid delimeter: delimeter must not be present in value {}".format(choice[1]))
            if choice[0] in key_to_value:
                raise KeyError("key {} has more than one values".format(choice[0]))
